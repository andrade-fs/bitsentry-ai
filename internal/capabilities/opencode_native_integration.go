...
func buildBitsentryAgentPrompt() string {
	return strings.Join([]string{
		...
		"- For any non-trivial repository/product/code/documentation/UX/security/bug request, your FIRST visible response MUST be a Route Decision Preview.",
		...
		"- Only AFTER showing that preview may you inspect files, analyze repo context, ask clarifying questions, recommend flows/capabilities, or propose changes.",
		...
		"- Before any non-trivial repository discovery, analysis, planning, flow recommendation, capability recommendation, or implementation proposal, you MUST show a concise Route Decision Preview visible to the user.",
		"- The preview must include: matched_intent, decision, recommended_flow/recommended_roles/recommended_skills (when applicable), matched_signals (when available), confidence (when available), reason, requires_confirmation, requires_bounded_discovery, and gates.",
